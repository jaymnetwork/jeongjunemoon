...
ggplot(gamedata, aes(x=pts))+
  geom_histogram(color="#ebebeb", fill="#108A99")+
...
court_points <- read.csv("courtPoints.csv") 
...
polar2<-function(data,col,dir){
  ...
  theme(
        ...
        plot.title = element_text(size=8, face="bold")) +
    scale_fill_viridis_d(option = col,direction = dir) +
    ggtitle(data$namePlayer)+
...
ld2<-polar2(doncic_shots,"E",1)
dl2<-polar2(lillard_shots,"B",1)
dm2<-polar2(mitchell_shots,"C",1)
je2<-polar2(embiid_shots,"D",1)
...
shot_line<-function(data,title){
  ...
  geom_line(aes(x=previousShot,y = ShotPercentage, group = 1), color = 'darkorange',
            size=0.6) +
...
```{r echo=F}
# Lillard
mod_forest1 <- randomForest(formula=form_def, data=lillard_shots3, ntree = 1000) ; mod_forest1
# Doncic
mod_forest2 <- randomForest(formula=form_def, data=doncic_shots3, ntree = 1000) ; mod_forest2
# Mitchell
mod_forest3 <- randomForest(formula=form_def, data=mitchell_shots3, ntree = 1000) ; mod_forest3
# Embiid
mod_forest4 <- randomForest(formula=form_def, data=embiid_shots3, ntree = 1000) ; mod_forest4
```
...
From the summary results, all four variables used in the definition had high p-values. However, *distancShot* variable was statistically significant for all four players. Note that as the coefficient is negative, we recognized that as a player shoots further to the basket, the player is likely to miss the shot, i.e., if a player gets closer shot, the player will have a higher chance of making the shot.
